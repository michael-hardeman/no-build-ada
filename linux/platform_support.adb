...
package body Platform_Support is
   ...
   --  O_WRONLY 1, O_CREAT 0100, O_TRUNC 01000.
   ...
   --  A 32-bit st_mode at byte 24, so the low half of word 4.
   function Stat_Mode (Words : Word_Array) return Integer is
   begin
      return Integer ((Words (4) mod 4294967296) mod 65536);
   end Stat_Mode;

   --  st_mtim.tv_sec at byte 88; the next word is the nanoseconds.
   function Stat_Mtime_Word return Integer is
   begin
      return 12;
   end Stat_Mtime_Word;

   --  d_ino 8, d_off 8, d_reclen 2, then d_type at byte 19,
   --  one-based.
